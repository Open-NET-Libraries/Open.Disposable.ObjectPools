	...
	where T : class
{
	...
	/// <inheritdoc />
	public override bool Recycle(T item) => _bin?.Writer.TryWrite(item) ?? false;

	protected override void OnCloseRequested() => _bin?.Writer.Complete();
...
public static class Recycler
{
	...
	public static Recycler<T> CreateRecycler<T>(
		this IObjectPool<T> pool,
		ushort limit = Constants.DEFAULT_CAPACITY)
		where T : class, IRecyclable => new(pool, Recycle, limit);
}
